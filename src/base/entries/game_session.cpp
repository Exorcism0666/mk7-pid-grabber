            ...
            if (option != UINT32_MAX)
            {
                ...
                if (player.loaded)
                {
                    ...
                    if (clean_pid)
                    {
                        ...
                        {
                            case decltype(pid_display.mode)::Decimal:
                                message_box += utilities::format_output("Principal ID", std::format("{:d}", principal_id), spoofed);
                            break;

                            case decltype(pid_display.mode)::Hexadecimal:
                                message_box += utilities::format_output("Principal ID", std::format("0x{:X}", principal_id), spoofed);
                            break;

                            case decltype(pid_display.mode)::Both:
                                message_box += utilities::format_output("Principal ID (DEC)", std::format("{:d}", principal_id), spoofed);
